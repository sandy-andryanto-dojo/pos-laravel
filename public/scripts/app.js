...
var dataTableRender = function(option) {
    ...
    if (option.tableName) {
        ...
        let routeName = option.routeName;
        let tableName = option.tableName;
        let dataTableConteiner = option.dataTableConteiner;
        let modelData = option.modelData;
        let dataTableColumns = option.dataTableColumns;

        $(dataTableConteiner).DataTable({
            'processing': true,
            'serverSide': true,
            'ajax': {
                'url': BASE_URL + '/api/datatable/' + routeName + '/' + modelData,
                'type': 'POST',
                'headers': {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                    'Authorization': 'Bearer ' + API_TOKEN,
            ...
            "order": [
                [dataTableColumns.length - 1, 'desc']
            ]
        });
        ...
        $("body").on("click", ".btn-delete", function(e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: BASE_URL + "/api/remove/" + modelData + "/" + id,
                        success: function(data) {
                            ...
                            $(dataTableConteiner).DataTable().ajax.reload();
                        },
                        error: function(data) {
                            console.log(data);
                        }
